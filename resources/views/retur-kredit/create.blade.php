...
@section('page-title', 'Tambah Retur Penjualan | TRANSDIGITAL - Pengelolaan Administrasi dan Transaksi Bisnis')

@section('section-heading', 'Retur Penjualan')
@section('section-row')
    ...
    <div class="card-body">
        ...
        <form action="{{ route('retur.store-penjualan') }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        <label class="form-label">Nama Debitur <span class="text-danger">*</span></label>
            ...
            <div class="mb-3">
                ...
                <textarea name="retur_keterangan" class="form-control" rows="2" placeholder="Contoh: Barang rusak saat pengiriman">{{ old('retur_keterangan') }}</textarea>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="retur_penanganan" class="form-select" required>
                    <option value="kurangi_piutang" {{ old('retur_penanganan') == 'kurangi_piutang' ? 'selected' : '' }}>
                        Kurangi Piutang (Kredit ke Piutang Usaha)
                    </option>
                    <option value="tunai_kembali" {{ old('retur_penanganan') == 'tunai_kembali' ? 'selected' : '' }}>
                        Kembalikan Tunai (Kredit ke Kas Utama)
                    </option>
                </select>
            </div>

            <hr>
            <h6 class="fw-bold mb-3">Item yang Diretur</h6>
                        ...
                        <tr>
                            <td>
                                <select name="items[0][barang_id]" class="form-select barang-select" required>
                                    ...
                                    @foreach($barang as $b)
                                        <option value="{{ $b->id }}" data-harga="{{ $b->harga_jual_per_unit }}">
                                            {{ $b->nama }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            <div class="text-end">
                ...
                <button type="submit" class="btn btn-success text-white">Simpan Retur</button>
            </div>
        </form>
    </div>
</div>
