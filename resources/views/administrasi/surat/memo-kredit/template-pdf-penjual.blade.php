...
<head>
    <meta charset="UTF-8">
    <title>Memo Kredit dari Penjual (Retur Pembelian) - {{ $retur->nomor_retur }}</title>
    <style>
        body {
...
<body>
    ...
    <table class="table-no-border mb-4">
        <tr>
            ...
            <td width="45%">
                ...
                {{ $retur->nomor_retur ?? '-' }}<br>

                <strong>Tanggal Retur</strong> :
                {{ \Carbon\Carbon::parse($retur->tanggal)->translatedFormat('d F Y') }}<br>

                <strong>No. Pesanan (SPP)</strong> :
                {{ $spp?->nomor_pesanan_pembelian ?? '-' }}<br>
                
                <strong>Tanggal Pesanan</strong> :
                {{ $spp?->tanggal_pesanan ? \Carbon\Carbon::parse($spp->tanggal_pesanan)->translatedFormat('d F Y') : '-' }}<br>
            </td>
        </tr>
    </table>

    <p class="mb-3">
        Bersama ini kami sampaikan rincian barang yang dikembalikan (retur pembelian) sebagai berikut:
    </p>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            {{-- spacer --}}
            @for ($i = count($retur->detail) + 1; $i <= 8; $i++)
                <tr>
                    <td>&nbsp;</td>
                    <td></td>
                    <td></td>
    ...
    <table class="table-no-border mb-4">
        <tr>
            ...
            <td width="75%"> {{ $retur->alasan_pengembalian ?? '-' }}</td>
        </tr>
    </table>
    ...
    <table width="100%" class="table-no-border" style="margin-top:60px;">
        <tr>
            ...
            <td width="50%" class="text-center">
                {{ \Carbon\Carbon::parse($retur->tanggal)->translatedFormat('d F Y') }}<br>
                Bagian Pembelian<br><br><br><br><br>
                <strong>( {{ $profileUser->nama_pimpinan ?? '_________' }} )</strong>
            </td>
        </tr>
    </table>
