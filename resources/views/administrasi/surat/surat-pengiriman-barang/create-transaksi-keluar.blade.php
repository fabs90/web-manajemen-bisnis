...
@section('page-title', 'Input Surat Pengiriman Barang (SPB) dari Supplier | Digitrans')
            ...
            <div class="card-body py-4">
                <form action="{{ route('administrasi.spb.store') }}" method="POST">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-8">
                            <label class="form-label fw-bold">Pilih Pesanan Pembelian (SPP) <span
                            ...
                            <select name="spp_id" id="spp_id" class="form-select @error('spp_id') is-invalid @enderror"
                                ...
                                @foreach ($dataSpp as $spp)
                                    @php
                                        $supplierNama = $spp->supplier?->nama ?? 'Supplier tidak tersedia';
                                        $supplierAlamat = $spp->supplier?->alamat ?? '-';
                                        $detailsJson = $spp->pesananPembelianDetail
                                            ...
                                            ->toJson();
                                    @endphp
                                    <option value="{{ $spp->id }}" data-pelanggan="{{ $supplierNama }}"
                                        data-alamat="{{ $supplierAlamat }}" data-nomor="{{ $spp->nomor_pesanan_pembelian }}"
                                        data-details="{{ $detailsJson }}">
                                        {{ $spp->nomor_pesanan_pembelian }} - {{ $supplierNama }}
                                    </option>
                                @endforeach
                            </select>
                            @error('spp_id')
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="jenis_pengiriman" class="form-control"
                                value="{{ old('jenis_pengiriman') }}" placeholder="Darat(JNT),Kargo" required>
                    ...
                    <div class="table-responsive mb-4">
                        <table class="table table-bordered table-sm">
                            ...
                            <tbody id="tabel-barang">
                                <tr>
                                    <td colspan="5" class="text-center text-muted">Pilih SPP terlebih dahulu...</td>
                                </tr>
                            </tbody>
                        </table>
            ...
            document.querySelectorAll('.checkbox-sesuai').forEach(chk => {
                chk.addEventListener('change', function() {
                    ...
                    const input = document.querySelector(
                        `input[name="items[${idx}][jumlah_dikirim]"]`);
