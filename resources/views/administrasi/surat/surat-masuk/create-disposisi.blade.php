...
@section('page-title', 'Disposisi Surat Masuk | TRANSDIGITAL - Administrasi Surat')
...
@section('section-row')
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('administrasi.surat-masuk.disposisi.store', ['id' => $surat->id]) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <table class="table table-bordered">
                        <tr>
                            <th colspan="2" class="text-center">LEMBAR DISPOSISI</th>
                        </tr>

                        <tr>
                            <td width="200px">Nomor Agenda Surat Masuk</td>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div><input type="checkbox" name="disposisi_status[]" value="Segera"
                                    {{ $surat->disp_segera ? 'checked' : '' }}> Segera</div>

                            <div><input type="checkbox" name="disposisi_status[]" value="Teliti dan beri pendapat"
                                    {{ $surat->disp_teliti ? 'checked' : '' }}> Teliti dan beri pendapat</div>

                            <div><input type="checkbox" name="disposisi_status[]" value="Edarkan"
                                    {{ $surat->disp_edarkan ? 'checked' : '' }}> Edarkan</div>

                            <div><input type="checkbox" name="disposisi_status[]" value="Untuk diketahui"
                                    {{ $surat->disp_diketahui ? 'checked' : '' }}> Untuk diketahui</div>
                        </div>

                        <div class="col-md-4">
                            <div><input type="checkbox" name="disposisi_status[]" value="Koordinasikan"
                                    {{ $surat->disp_koordinasikan ? 'checked' : '' }}> Koordinasikan</div>

                            <div><input type="checkbox" name="disposisi_status[]" value="Proses lebih lanjut"
                                    {{ $surat->disp_proses_lanjut ? 'checked' : '' }}> Proses lebih lanjut</div>

                            <div><input type="checkbox" name="disposisi_status[]" value="Arsipkan"
                                    {{ $surat->disp_arsipkan ? 'checked' : '' }}> Arsipkan</div>

                            <div><input type="checkbox" name="disposisi_status[]" value="Mohon dijawab"
                    ...
                    <div>
                        <div><input type="checkbox" name="tujuan_status[]" value="Keuangan"
                                {{ $surat->tujuan_keuangan ? 'checked' : '' }}> Keuangan</div>

                        <div><input type="checkbox" name="tujuan_status[]" value="Kepala Bagian Gudang"
                                {{ $surat->tujuan_gudang ? 'checked' : '' }}> Ka. Bagian Gudang</div>

                        <div><input type="checkbox" name="tujuan_status[]" value="Karyawan"
                                {{ $surat->tujuan_karyawan ? 'checked' : '' }}> Karyawan</div>

                        <div><input type="checkbox" name="tujuan_status[]" value="Lainnya"
                    ...
                    @if ($surat->ttd_pimpinan)
                        <div class="mt-3">
                            ...
                            <img src="{{ asset('storage/' . $surat->ttd_pimpinan) }}" alt="TTD Pimpinan"
