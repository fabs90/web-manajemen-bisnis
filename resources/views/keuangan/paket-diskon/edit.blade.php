...
@section('page-title', 'Edit Paket Diskon | TRANSDIGITAL')
@section('section-heading', 'Edit Paket Diskon Penjualan')
@section('section-row')
<div class="card shadow-sm">
    <div class="card-body">
        ...
        <form action="{{ route('keuangan.paket-diskon.update', $paketDiskon->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="nama_paket" class="form-control" value="{{ old('nama_paket', $paketDiskon->nama_paket) }}" required placeholder="Contoh: Promo Lebaran">
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="barang_id" class="form-select">
                        ...
                        @foreach ($barang as $b)
                            <option value="{{ $b->id }}" {{ old('barang_id', $paketDiskon->barang_id) == $b->id ? 'selected' : '' }}>
                                {{ $b->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="jenis_diskon" id="jenis_diskon" class="form-select" required>
                        <option value="persentase" {{ old('jenis_diskon', $paketDiskon->jenis_diskon) == 'persentase' ? 'selected' : '' }}>Persentase (%)</option>
                        <option value="nominal" {{ old('jenis_diskon', $paketDiskon->jenis_diskon) == 'nominal' ? 'selected' : '' }}>Nominal (Rp)</option>
                    </select>
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label">Nilai Diskon <span class="text-danger">*</span></label>
                    <input type="text" name="nilai_diskon" id="nilai_diskon" class="form-control" value="{{ old('nilai_diskon', $paketDiskon->jenis_diskon === 'nominal' ? number_format($paketDiskon->nilai_diskon, 0, ',', '.') : round($paketDiskon->nilai_diskon, 0)) }}" required>
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label">Minimal Pembelian (Rp) <span class="text-muted">(Opsional)</span></label>
                    <input type="text" name="minimal_pembelian" id="minimal_pembelian" class="form-control rupiah" value="{{ old('minimal_pembelian', number_format($paketDiskon->minimal_pembelian, 0, ',', '.')) }}">
                    <small class="text-muted">Isi 0 jika tidak ada batas minimal pembelian.</small>
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label d-block">Status Aktif</label>
                    <div class="form-check form-switch mt-2">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $paketDiskon->is_active) ? 'checked' : '' }}>
            ...
            <div class="mt-3 text-end">
                ...
                <button type="submit" class="btn btn-primary">Update Paket Diskon</button>
            </div>
        </form>
    </div>
</div>
