...
@section('page-title', 'Input Pendapatan')
...
<div class="card shadow-sm">
    <div class="card-body">
        ...
        <form action="{{ route('keuangan.pendapatan.store') }}" method="POST">
            ...
            {{-- Uraian Pendapatan --}}
            <div class="mb-3">
                <label for="uraian_pendapatan" class="form-label">Uraian Pendapatan<span class="text-danger">*</span></label>
                <input type="text" name="uraian_pendapatan" id="uraian_pendapatan" class="form-control @error('uraian_pendapatan') is-invalid @enderror" value="{{ old('uraian_pendapatan') }}" placeholder="Uraian Pendapatan" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="jumlah" id="jumlah" class="form-control rupiah @error('jumlah') is-invalid @enderror" value="{{ old('jumlah') }}" placeholder="Jumlah Penjualan" required>
            ...
            <div class="mb-3">
                ...
                <select name="jenis_pendapatan" id="jenis_pendapatan" class="form-select @error('jenis_pendapatan') is-invalid @enderror" required>
                    <option value="" disabled {{ old('jenis_pendapatan') ? '' : 'selected' }}>-- Pilih Jenis --</option>
                    <option value="tunai" {{ old('jenis_pendapatan') == 'tunai' ? 'selected' : '' }}>Tunai</option>
                    <option value="piutang" {{ old('jenis_pendapatan') == 'piutang' ? 'selected' : '' }}>Piutang</option>
                    <option value="kredit" {{ old('jenis_pendapatan') == 'kredit' ? 'selected' : '' }}>Kredit (Pelunasan)</option>
                </select>
                @error('jenis_pendapatan') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            {{-- Checkbox: Apakah ada debitur? --}}
            <div id="debitur-check-container" class="mb-3 d-none">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="ada_debitur_check" name="ada_debitur_check" value="1" {{ old('ada_debitur_check') ? 'checked' : '' }}>
            ...
            <div id="debitur-section" class="d-none">
                ...
                <div class="mb-3">
                    <label for="nama_pelanggan" class="form-label">Nama Debitur<span class="text-danger">*</span></label>
                    <select name="nama_pelanggan" id="nama_pelanggan" class="form-control @error('nama_pelanggan') is-invalid @enderror" disabled>
                        <option value="" disabled {{ old('nama_pelanggan') ? '' : 'selected' }}>-- Pilih Debitur --</option>
                        @foreach ($debitur as $pelanggan)
                            <option value="{{ $pelanggan->id }}" {{ old('nama_pelanggan') == $pelanggan->id ? 'selected' : '' }}>{{ $pelanggan->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div id="piutang_aktif_container" class="mb-3 d-none">
                    ...
                    <select name="piutang_aktif" id="piutang_aktif" class="form-control" disabled>
                        ...
                        @foreach ($listPiutang as $piutang)
                            <option value="{{ $piutang->kode }}" data-debitur="{{ $piutang->pelanggan_id }}" class="piutang-option">
                ...
                <div id="hutang_aktif_container" class="mb-3 d-none">
                    ...
                    <select name="hutang_aktif" id="hutang_aktif" class="form-control" disabled>
                        ...
                        @foreach ($listPiutang as $piutang)
                            <option value="{{ $piutang->kode }}" data-debitur="{{ $piutang->pelanggan_id }}" class="hutang-option">
            ...
            <div class="mb-3">
                ...
                <div class="form-check mb-2">
                    <input class="form-check-input" type="checkbox" id="barang_terjual_check" name="barang_terjual_check" value="1" {{ old('barang_terjual_check') ? 'checked' : '' }}>
                    <label class="form-check-label" for="barang_terjual_check">Tidak ada barang terjual</label>
                </div>
                <select name="barang_terjual" id="barang_terjual" class="form-control @error('barang_terjual') is-invalid @enderror">
                    <option value="" disabled {{ old('barang_terjual') ? '' : 'selected' }}>-- Pilih Barang --</option>
                    @foreach ($barang as $item)
                        <option value="{{ $item->id }}" {{ old('barang_terjual') == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                    @endforeach
                </select>
                @error('barang_terjual') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="mb-3">
                <label for="jumlah_barang_dijual" class="form-label">Jumlah Barang (satuan)</label>
                <input type="number" name="jumlah_barang_dijual" id="jumlah_barang_dijual" class="form-control @error('jumlah_barang_dijual') is-invalid @enderror" value="{{ old('jumlah_barang_dijual') }}"  >
                @error('jumlah_barang_dijual') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            {{-- Biaya Tambahan --}}
            <div class="mb-3">
                <label for="potongan_pembelian" class="form-label">Potongan</label>
                <input type="text" name="potongan_pembelian" id="potongan_pembelian" class="form-control rupiah @error('potongan_pembelian') is-invalid @enderror" value="{{ old('potongan_pembelian') }}" placeholder="Biaya Tambahan (Opsional)">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="biaya_lain" id="biaya_lain" class="form-control rupiah @error('biaya_lain') is-invalid @enderror" value="{{ old('biaya_lain') }}" placeholder="Biaya Lain (Opsional)">
                @error('biaya_lain') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="mb-3">
                <label for="bunga_bank" class="form-label">Bunga Bank</label>
                <input type="text" name="bunga_bank" id="bunga_bank" class="form-control rupiah @error('bunga_bank') is-invalid @enderror" value="{{ old('bunga_bank') }}" placeholder="Bunga Bank (Opsional)">
