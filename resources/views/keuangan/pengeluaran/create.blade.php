            ...
            <form action="{{ route('keuangan.pengeluaran.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="jenis_pengeluaran" id="jenis_pengeluaran"
                        ...
                        <option value="" disabled {{ old('jenis_pengeluaran') == null ? 'selected' : '' }}>-- Pilih
                            Jenis Pengeluaran --</option>
                ...
                <div id="kreditur-section" class="d-none">
                    ...
                    <div class="mb-3">
                        <label for="nama_kreditur" class="form-label">Nama Kreditur<span
                                class="text-danger">*</span></label>
                        <select name="nama_kreditur" id="nama_kreditur"
                            class="form-control @error('nama_kreditur') is-invalid @enderror" disabled>
                            <option value="" disabled {{ old('nama_kreditur') == null ? 'selected' : '' }}>-- Pilih
                                Kreditur --</option>
                            @isset($kreditur)
                                @foreach ($kreditur as $supplier)
                                    <option value="{{ $supplier->id }}"
                                        {{ old('nama_kreditur') == $supplier->id ? 'selected' : '' }}>{{ $supplier->nama }}
                                    </option>
                                @endforeach
                            @endisset
                        </select>
                ...
                <div id="hutang-section" class="d-none">
                    <div class="mb-3">
                        ...
                        <select name="hutang_id" id="hutang_id"
                            class="form-control @error('hutang_id') is-invalid @enderror">
                            <option value="" disabled {{ old('hutang_id') == null ? 'selected' : '' }}>-- Pilih
                                Hutang --</option>
                            @isset($listHutang)
                                @foreach ($listHutang as $hutang)
                                    <option value="{{ $hutang->id }}"
                                        {{ old('hutang_id') == $hutang->id ? 'selected' : '' }}>
                                        {{ $hutang->uraian }} - Rp {{ number_format($hutang->saldo, 0, ',', '.') }} -
                        ...
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" id="ada_barang_baru"
                                {{ old('ada_barang_baru') ? 'checked' : '' }}>
                            <label class="form-check-label" for="ada_barang_baru">Apakah ada barang baru yang
                                dibeli?</label>
                        </div>
                    </div>
                </div>
                ...
                <div id="barang-section" class="d-none ms-3 mb-3">
                    <div class="mb-3">
                        <label for="barang_dibeli" class="form-label">Barang Dibeli<span
                                class="text-danger">*</span></label>
                        <select name="barang_dibeli" id="barang_dibeli"
                            class="form-control @error('barang_dibeli') is-invalid @enderror">
                            <option value="" disabled {{ old('barang_dibeli') == null ? 'selected' : '' }}>-- Pilih
                                Barang --</option>
                            @isset($barang)
                                @foreach ($barang as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('barang_dibeli') == $item->id ? 'selected' : '' }}>{{ $item->nama }} - Rp
                                        {{ number_format($item->harga_jual_per_unit, 0, ',', '.') }}</option>
                                @endforeach
                            @endisset
                        </select>
                        @error('barang_dibeli')
                    ...
                    <div class="mb-3">
                        <label for="jumlah_barang_dibeli" class="form-label">Jumlah Barang<span
                                class="text-danger">*</span></label>
                        <input type="number" name="jumlah_barang_dibeli" id="jumlah_barang_dibeli"
                            class="form-control @error('jumlah_barang_dibeli') is-invalid @enderror"
                            value="{{ old('jumlah_barang_dibeli') }}">
